#!/usr/bin/env python3
"""
Executa pyproteum para cada caso de teste que passou na baseline
e salva o Mutation Score por caso de teste em JSON.

Pipeline por teste:
  1. python3 -m pyproteum testnew --D . --S <fonte>.py <sessao>
  2. python3 -m pyproteum tcase --add --S <teste>.py <sessao>
  3. python3 -m pyproteum mutagen --create <sessao>
  4. python3 -m pyproteum exemuta --exec <sessao>

No Windows, o script se re-invoca automaticamente via WSL.
"""
...
TIMEOUT_BASELINE = 30    # segundos
TIMEOUT_TESTNEW = 30
TIMEOUT_TCASE = 30
TIMEOUT_MUTAGEN = 120
TIMEOUT_EXEMUTA = 300    # pode ser longo dependendo do numero de mutantes
...
def parsear_nome_teste(filename):
    """Extrai (llm, prompt_type) do nome do arquivo de teste."""
    base = filename[5:-3]  # remove 'test_' e '.py'
...
def verificar_baseline(task_path, test_file):
    """Retorna True se o teste passa contra o codigo original."""
    env = os.environ.copy()
...
def parsear_score(output):
    """
    Extrai mutation score da saida do pyproteum.
    Suporta formatos em portugues e ingles.
    Retorna dict {score, killed, survived, total} ou None.
    """
    # Padrao 1 — Score de Mutacao: X% (portugues)
    m = re.search(
    ...
    # Padrao 2 — Mutation Score: X% (ingles)
    ...
    # Padrao 3 — "Mutantes mortos / vivos" (portugues)
    ...
    # Padrao 4 — "Killed / Survived" (ingles)
    ...
    # Padrao 5 — "X of Y mutants killed"
...
def rodar_pyproteum(task_path, source_file, test_file):
    """
    Copia os arquivos para um diretorio temporario e executa o pipeline
    completo do pyproteum. Retorna (dict_resultado | None, raw_output).
    """
    session_name = test_file[:-3]   # ex: test_CodeGemma_few_shot
    nome_funcao = source_file[:-3]  # ex: intersperse

    with tempfile.TemporaryDirectory() as tmp:
        # Copiar arquivo fonte
        shutil.copy(os.path.join(task_path, source_file), os.path.join(tmp, source_file))

        # Copiar e ajustar imports do arquivo de teste
        ...
        # Preferir saida do exemuta; fallback para output completo
...
def main():
    ...
    for idx, task_dir in enumerate(task_dirs):
        ...
        # Checkpoint parcial apos cada task
